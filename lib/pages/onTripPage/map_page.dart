...
// dynamic _currentCenter;
  ...
  bool _loading = false;
  /// While requesting location; do not drive full-screen [Loading] with this — it blocks the OS permission dialog.
  ...
  void initState() {
    ...
    if (mapType.isEmpty && userDetails.isNotEmpty) {
      final mt = userDetails['map_type'];
      mapType = (mt != null && mt.toString().trim().isNotEmpty)
          ? mt.toString().trim()
          : 'google';
    }
...
//get location permission and location details
  ...
  Future<void> _getLocsImpl() async {
    ...
    // --- Permission not granted yet — show location onboarding (state '2').
    if (permission == geolocs.LocationPermission.denied) {
      gettingPerm++;
      if (gettingPerm > 1 || identical(locationAllowed, false)) {
        state = '3';
    ...
    // --- Blocked in OS settings — keep onboarding; Next handler can open Settings.
    ...
    // --- Permission OK — GPS must still be enabled (do NOT bump gettingPerm here).
    // Web has no Android-style "location services" toggle; the API often reports
    // false and blocks the map even when the browser can provide coordinates.
    if (kIsWeb) {
      serviceEnabled = true;
    } else {
      serviceEnabled = await geolocs.Geolocator.isLocationServiceEnabled();
    ...
    // --- Ready to read coordinates.
    // On web, getLastKnownPosition is a common failure/hang point; use a timed
    // current position instead (see _resolveInitialPosition).
    final geolocs.Position locs = await _resolveInitialPosition();
  ...
  Future<geolocs.Position> _resolveInitialPosition() async {
    const acquireTimeout = Duration(seconds: 45);
    if (kIsWeb) {
      return geolocs.Geolocator.getCurrentPosition(
        desiredAccuracy: geolocs.LocationAccuracy.low,
      ).timeout(acquireTimeout);
    }
    try {
      final cached = await geolocs.Geolocator.getLastKnownPosition().timeout(
        const Duration(seconds: 5),
      );
      if (cached != null) return cached;
    } on TimeoutException {
      // Unavailable or slow; fetch a fresh fix below.
    }
    return geolocs.Geolocator.getCurrentPosition(
      desiredAccuracy: geolocs.LocationAccuracy.low,
    ).timeout(acquireTimeout);
  ...
  getLocationPermission() async {
    ...
    try {
      // Request OS permission first — matches Android/iOS dialogs reliably.
      await perm.Permission.location.request();

      permission = await geolocs.Geolocator.checkPermission();
      if (permission == geolocs.LocationPermission.denied) {
        permission = await geolocs.Geolocator.requestPermission();
      }
      if (permission == geolocs.LocationPermission.deniedForever) {
        await geolocs.Geolocator.openAppSettings();
      ...
      var gpsOn =
          kIsWeb || await geolocs.Geolocator.isLocationServiceEnabled();
      if (!kIsWeb && !gpsOn) {
        await location.requestService();
        await Future.delayed(const Duration(milliseconds: 600));
        gpsOn = await geolocs.Geolocator.isLocationServiceEnabled();
      }
      serviceEnabled = gpsOn;
      ...
      debugPrint('getLocationPermission error: $e\n$st');
      if (mounted) {
        setState(() {
          state = '2';
        });
                                                                        ...
                                                                        SizedBox(
                                                                          ...
                                                                          child:
                                                                            ...
                                                                            child:
                                                                              ...
                                                                              children: [
                                                                                (userDetails['show_rental_ride'] == true || userDetails['enable_modules_for_applications'] == 'both')
...
//loader — never above permission prompts (would block OS dialogs / touches).
                      ((_loading == true || state == '') &&
                              state != '1' &&
                              state != '2')
