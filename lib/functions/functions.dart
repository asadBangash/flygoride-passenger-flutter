...
import 'package:flutter_user/config/app_env.dart';
...
bool internet = true;
/// Set after `Firebase.initializeApp` succeeds (see [main.dart]).
...
// API base — override with: flutter build apk --dart-define=API_BASE_URL=https://...
String url = AppEnv.normalizedApiBase();
String mapkey =
...
darktheme() async {
  // Rider app identity: black surfaces + green accents (both theme toggle modes).
  page = Colors.black;
  ...
  buttonColor = theme;
  loaderColor = theme;
  hintColor = Colors.white.withOpacity(0.38);
  boxshadow = BoxShadow(
      blurRadius: 2,
...
/// FlyGoRide passenger app: English, Urdu, Pashto only (first-run language picker).
const List<String> flygorideLanguageCodes = ['en', 'ur', 'ps'];
...
userLogin(number, login, password, isOtp) async {
  ...
  try {
    ...
    final Map<String, dynamic> body = {
      ...
      if (isOtp == false && login == 1) 'email': number,
      if (isOtp == true && login == 0) 'mobile': number,
      if (isOtp == true && login == 1) 'email': number,
      if (isOtp == false) 'password': password,
      if (isOtp == true && login == 1) 'otp': password,
      'login_by':
          (platform == TargetPlatform.android) ? 'android' : 'ios',
    };
    if (fcmToken.isNotEmpty) body['device_token'] = fcmToken;
...
geoCoding(double lat, double lng) async {
  ...
  try {
    ...
    printWrapped(val.body.toString());
    if (val.statusCode == 200) {
      ...
      printWrapped('address is $result');
      return result;
    }
  } catch (e) {
    if (e is SocketException) {
...
outStationListFun() async {
  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      printWrapped(outStationList.toString());
      result = 'success';
      valueNotifierBook.incrementNotifier();
    } else if (response.statusCode == 401) {
      result = 'logout';
